...
static inline Janet jutf8_conv_case(int argc, Janet *argv, int up) {
  ...
  for (utf8proc_ssize_t i = 0; i < l; ) {
    ...
    if (r == -1)
      janet_panicf("%v is not a valid utf8 string: %s", argv[0], utf8proc_errmsg(sz));
    i += sz;
    utf8proc_uint8_t buf[4];
    utf8proc_ssize_t n = utf8proc_encode_char(up ? utf8proc_toupper(r) : utf8proc_tolower(r), buf);
    if (n > sz) {
      // Fairly sure this holds true.
      janet_panicf("janet-utf8-bug: utf8 case conversion increased string length (file an issue)");
    }
...
static inline Janet jutf8_normalize2(int argc, Janet *argv, utf8proc_option_t options) {
  janet_fixarity(argc, 1);
  JanetByteView bv = janet_getbytes(argv, 0);
  Janet v;
  utf8proc_uint8_t *s = NULL;
  utf8proc_int32_t slen = utf8proc_map(bv.bytes, bv.len, &s,  UTF8PROC_STABLE | UTF8PROC_COMPOSE);
  if (slen < 0)
...
static Janet jutf8_normalize_NFKD(int argc, Janet *argv) {
  return jutf8_normalize2(argc, argv, UTF8PROC_STABLE | UTF8PROC_DECOMPOSE | UTF8PROC_COMPAT);
}

static Janet jutf8_normalize_NFKC(int argc, Janet *argv) {
  return jutf8_normalize2(argc, argv, UTF8PROC_STABLE | UTF8PROC_COMPOSE | UTF8PROC_COMPAT);
}

static Janet jutf8_normalize_NFKC_Casefold(int argc, Janet *argv) {
  return jutf8_normalize2(argc, argv, UTF8PROC_STABLE | UTF8PROC_COMPOSE | UTF8PROC_COMPAT | UTF8PROC_CASEFOLD | UTF8PROC_IGNORE);
}

static const JanetReg cfuns[] = {
    {"valid?", jutf8_valid, "(utf8/valid? s)\n\nReturn true if s is a valid utf8 character sequence, otherwise false."},
    {"to-lower", jutf8_to_lower, "(utf8/to-lower s)\n\nReturn the lower case conversion of s or raise an error if s invalid utf8."},
    {"to-upper", jutf8_to_upper, "(utf8/to-upper s)\n\nReturn the upper case conversion of s or raise an error if s invalid utf8."},
    {"to-codepoints", jutf8_to_codepoints, "(utf8/to-codepoints s)\n\nReturn an array containing s split into codepoints as janet numbers."},
    {"normalize", jutf8_normalize_NCF, "(utf8/normalize s)\n\nReturn the utf8 NFC normalization of s."},
    {"normalize-NCF", jutf8_normalize_NCF, "(utf8/normalize-NCF s)\n\nReturn the utf8 NFC normalization of s."},
    {"normalize-NFD", jutf8_normalize_NFD, "(utf8/normalize-NFD s)\n\nReturn the utf8 NFD normalization of s."},
    {"normalize-NFKD", jutf8_normalize_NFKD, "(utf8/normalize-NFKD s)\n\nReturn the utf8 NFKD normalization of s."},
    {"normalize-NFKC", jutf8_normalize_NFKC, "(utf8/normalize-NFKC s)\n\nReturn the utf8 NFKC normalization of s."},
    {"normalize-NFKC-casefold", jutf8_normalize_NFKC_Casefold, "(utf8/normalize-NFKC-casefold s)\n\nReturn the utf8 NFKC-casefold normalization of s."},
    {NULL, NULL, NULL}};
